...
for E in $ETAS; do
  ...
  [ -f "$OUT" ] && { echo "=== LB2SWEEP eta=$E removing partial ($n records)"; rm -f "$OUT"; }
  echo "=== LB2SWEEP eta=$E START $(date -Iseconds)"
  CUDA_VISIBLE_DEVICES=0 $PY -m eval.LongBench2.pred \
    --model qwen-2.5-chat-14b --method spec_ret \
    --page_rep oval --eta "$E" --oval_rank 8 \
    --GQA_policy avgSM --spec_ret_steps 2 --spec_ret_corr 0.8 \
    --sink 128 --recent 128 --budget 1792 \
    --out_root_dir eval/LongBench2/res \
    > "$LOG/lb2_qwen14b_eta${E}.log" 2>&1
  rc=$?
  got=$([ -f "$OUT" ] && grep -c . "$OUT" || echo 0)
  echo "=== LB2SWEEP eta=$E DONE rc=$rc records=$got $(date -Iseconds)"
done
echo "=== LB2SWEEP ALL DONE $(date -Iseconds)"
